...
export async function POST() {
  try {
    ...
    // Try to create the ai_configurations table
    try {
      // First check if the table exists
      const { error: checkError } = await supabase
      ...
      if (checkError && checkError.message.includes('relation "ai_configurations" does not exist')) {
        // Table doesn't exist, create it using a simple query
        const { error: createError } = await supabase.rpc('create_table_if_not_exists', {
        ...
        if (createError) {
          console.error('Error creating ai_configurations table:', createError);
        } else {
          ...
          if (insertError) {
            console.error('Error inserting default AI configuration:', insertError);
          } else {
            console.log('Default AI configuration inserted successfully');
          }
        }
      } else {
        console.log('ai_configurations table already exists');
      }
    } catch (error) {
      console.error('Error handling ai_configurations table:', error);
    }
