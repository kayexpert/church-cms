...
/**
 * API route to create the monthly financial data function
 */
export async function GET() {
  try {
    ...
    // Read the SQL file
    const sqlFilePath = path.join(process.cwd(), "src", "db", "migrations", "monthly_financial_data_function.sql");
    ...
    try {
      ...
      if (error) {
        ...
        const { error: directError } = await supabase.from('_temp_migrations').insert({
          name: 'monthly_financial_data_function',
          sql: sqlContent
        });
        ...
        if (directError) {
          ...
          return NextResponse.json(
            { error: `Failed to create monthly data function: ${directError.message}` },
            { status: 500 }
          );
        }
      }
      ...
      try {
        ...
        console.error("Error executing SQL directly:", directError);
        return NextResponse.json(
          { error: `Failed to create monthly data function: ${directError instanceof Error ? directError.message : String(directError)}` },
          { status: 500 }
        );
      }
    }
    ...
    return NextResponse.json(
      { message: "Monthly data function created successfully" },
      { status: 200 }
    );
  } catch (error) {
    console.error("Error creating monthly data function:", error);
    return NextResponse.json(
      { error: `Failed to create monthly data function: ${error instanceof Error ? error.message : String(error)}` },
      { status: 500 }
    );
  }
}
