      ...
      try {
        ...
        if (transactionError) {
          ...
          results.push({
            ...
            success: false,
            error: transactionError instanceof Error ? transactionError.message : "Error fetching transactions"
          });
          continue;
        }
        ...
        if (transactions && transactions.length > 0) {
          ...
          calculatedBalance = calculateAccountBalance(account as any, transactions);
        ...
        if (updateError) {
          ...
          results.push({
            ...
            account_name: account.name,
            success: false,
            error: updateError instanceof Error ? updateError.message : "Error updating account balance"
          });
        } else {
          results.push({
            account_id: account.id,
