...
import { isSystemExpenditureCategory } from "@/lib/identify-system-categories";

// Function to identify system categories
const isSystemCategory = (category: ExpenseCategory): boolean => {
  // Check if it's a system category using the utility function
  return isSystemExpenditureCategory(category) ||
    // Additional checks specific to this component
    category.name === "Budget Allocation" ||
    category.name?.startsWith("Budget allocation for ") ||
...
export function ExpenseCategorySettings() {
  ...
  const editForm = useForm<ExpenseCategoryFormValues>({
    resolver: zodResolver(expenseCategorySchema),
    defaultValues: {
      name: "",
      description: "",
  ...
  const updateDisplayedCategories = (data: ExpenseCategory[], showSystem: boolean) => {
    if (showSystem) {
      setCategories(data);
    } else {
  ...
  const onSubmit = async (values: ExpenseCategoryFormValues) => {
    try {
      ...
      if (existingCategory) {
        toast.error("An expense category with this name already exists");
        return;
      }
  ...
  const onEditSubmit = async (values: ExpenseCategoryFormValues) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Check if category is being used by any expense entries
      const { data: expenseEntries, error: checkError } = await supabase
      ...
      if (expenseEntries && expenseEntries.length > 0) {
        toast.error(`Cannot delete category. It is used by ${expenseEntries.length} expense entries.`);
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-xl font-semibold mb-4">Add New Expense Category</h2>
        <Form {...form}>
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <label htmlFor="show-system-categories" className="text-sm cursor-pointer">
        ...
        {categories.length === 0 ? (
          <p className="text-muted-foreground">No expense categories added yet</p>
        ) : (
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete the expense category "{categoryToDelete?.name}"? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
